...
fn main() -> Result<(), Box<dyn Error> > {
    let csv_file = r##"F:\3M Promo Data\May1-July31 2020\data.csv"##;
    let json_promo = r##"F:\3M Promo Data\May1-July31 2020\promo_May1_2020-July31_2020.json"##;
    //let mut csv = CSVFile::new(csv_file, json_promo);
    //let complete_promo = csv.read();
    let completed_promo = load_promo( csv_file, json_promo);

    let mut ve = Vec::new();
    if let Ok(mut promo) = completed_promo {
        display_missing_report(&mut promo.data, &mut ve)?;
        if let Ok(s) = String::from_utf8(ve) {
            println!("{}", s);
        }
    }
    Ok(())
